...
mod test {
    ...
    // TS packet that doesn't start with sync byte.
    const NOT_SYNC_PACKET: &[u8] = &[
        0x48, 0x1f, 0xff, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    // Too short TS packet.
    const SHORT_PACKET: &[u8] = &[
        0x47, 0xFF
    ];

    // Some TS packet that contains some video data.
    const PACKET: &[u8] = &[
        0x47, 0x48, 0x12, 0x3C, 0x3E, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
        0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x01, 0xE0, 0x00, 0x00, 0x80, 0xC0, 0x0A, 0x31, 0x00, 0x01, 0x92,
        0x7D, 0x11, 0x00, 0x01, 0x3A, 0x99, 0x00, 0x00, 0x00, 0x01, 0x06, 0x01, 0x02, 0x01, 0x06, 0x80,
        0x00, 0x00, 0x01, 0x41, 0x9A, 0x22, 0x6C, 0x49, 0xFF, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00,
        ...
        0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x00, 0x15, 0xEA, 0x3B, 0xC4, 0xC3, 0xBF, 0x99, 0xD9,
        0xD7, 0xFB, 0xB4, 0x97, 0xA0, 0xE7, 0xC0, 0x7F, 0x8A, 0xBF, 0xA1, 0x6B, 0x24, 0x9E, 0xDE, 0x9B,
        0xA8, 0x29, 0x38, 0xD5, 0x80, 0x12, 0x31, 0xF7, 0x00, 0x00, 0xB6, 0x80
    ];
    ...
    const PAYLOAD: &[u8] = &[
                          ...
                          0x00, 0x00, 0x01, 0xE0, 0x00, 0x00, 0x80, 0xC0, 0x0A, 0x31, 0x00, 0x01, 0x92,
        0x7D, 0x11, 0x00, 0x01, 0x3A, 0x99, 0x00, 0x00, 0x00, 0x01, 0x06, 0x01, 0x02, 0x01, 0x06, 0x80,
        0x00, 0x00, 0x01, 0x41, 0x9A, 0x22, 0x6C, 0x49, 0xFF, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00,
        0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x00,
        0x03, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03,
    ...
    #[test]
    fn new() {
        let not_sync = TsPacket::new(NOT_SYNC_PACKET);
        assert_eq!(not_sync.is_err(), true);

        let short = TsPacket::new(SHORT_PACKET);
        assert_eq!(short.is_err(), true);

        let ts = TsPacket::new(PACKET);
        assert_eq!(ts.is_ok(), true)
    }

    #[test]
    fn is_error() {
        let ts = TsPacket::new(PACKET).unwrap();
        assert_eq!(ts.is_error(), false);
    }

    #[test]
    fn is_pusi() {
        let ts = TsPacket::new(PACKET).unwrap();
        assert_eq!(ts.is_pusi(), true);
    }

    #[test]
    fn get_pid() {
        let ts = TsPacket::new(PACKET).unwrap();
        assert_eq!(ts.get_pid(), 2066);
    }

    #[test]
    fn is_adaptation() {
        let ts = TsPacket::new(PACKET).unwrap();
        assert_eq!(ts.is_adaptation(), true);
    }

    #[test]
    fn is_payload() {
        let ts = TsPacket::new(PACKET).unwrap();
        assert_eq!(ts.is_payload(), true);
    }

    #[test]
    fn get_cc() {
        let ts = TsPacket::new(PACKET).unwrap();
        assert_eq!(ts.get_cc(), 12);
    }

    #[test]
    fn get_adaptation_size() {
        let ts = TsPacket::new(PACKET).unwrap();
        assert_eq!(ts.get_adaptation_size(), 62);
    }

    #[test]
    fn get_payload_offset() {
        let ts = TsPacket::new(PACKET).unwrap();
        assert_eq!(ts.get_payload_offset(), 67);
    }

    #[test]
    fn get_payload() {
        let ts = TsPacket::new(PACKET).unwrap();
        assert_eq!(ts.get_payload(), PAYLOAD);
    }

    #[test]
    fn is_pes() {
        let ts = TsPacket::new(PACKET).unwrap();
        assert_eq!(ts.is_pes(), true);
    }
}
